...
void config()
{
	...
	// Set spreading factor if SF11 or SF12
...
void receivepacket() {
    ...
    if(digitalRead(dio0) == 1)
    {
	    ...
	    fprintf(CSVfile, "\n%d; %d; %li; %s; %d; %d; %d; %d",readReg(0x1A)-rssicorr,readReg(0x1B)-rssicorr, SNR, message, freq_, bw_, power_, sf_);
	    fclose(CSVfile);	
...
void createLogFile(char* fileName)
{
	...
	CSVfile = fopen(filename , "w"); // Create empty file
	fprintf(CSVfile, "Packet RSSI; RSSI; SNR; Payload; Frequency; Bandwidth; Power Level; SF");
	fclose(CSVfile);
